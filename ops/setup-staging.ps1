...
$DbProd = Join-Path $Backend "lanxa.db"
if (-not (Test-Path $DbProd)) { $DbProd = Join-Path $Backend "superozono.db" }
$DbStaging = Join-Path $Backend "lanxa_staging.db"

Write-Host "== Lanxa - setup staging LAN ==" -ForegroundColor Cyan
...
if (Test-Path $DbStaging) {
    ...
    Write-Host "No hay lanxa.db local; staging arrancara vacia"
}
...
if (Test-Path $EnvStaging) {
    ...
    foreach ($line in $lines) {
        if ($line -match '^\s*DATABASE_URL\s*=') {
            $out.Add('DATABASE_URL=sqlite+aiosqlite:///./lanxa_staging.db')
            $hasDb = $true
    ...
    if (-not $hasDb) { $out.Insert(0, 'DATABASE_URL=sqlite+aiosqlite:///./lanxa_staging.db') }
...
Write-Host "  Backend:  set DATABASE_URL=sqlite+aiosqlite:///./lanxa_staging.db"
